...
func init() {
	...
	jsonExt.EncodeType(bson.Binary{}, jencBinaryType)
	...
	jsonExt.EncodeType(bson.MongoTimestamp(0), jencTimestamp)

	funcExt.DecodeConst("undefined", bson.Undefined)

	jsonExt.DecodeKeyed("$regex", jdecRegEx)
	jsonExt.EncodeType(bson.RegEx{}, jencRegEx)

	funcExt.DecodeFunc("ObjectId", "$oidFunc", "Id")
	jsonExt.DecodeKeyed("$oid", jdecObjectId)
	jsonExt.DecodeKeyed("$oidFunc", jdecObjectId)
	jsonExt.EncodeType(bson.ObjectId(""), jencObjectId)
	...
	funcExt.DecodeConst("MinKey", bson.MinKey)
	funcExt.DecodeConst("MaxKey", bson.MaxKey)
	...
	jsonExt.EncodeType(bson.Undefined, jencUndefined)
...
func jdecBinary(data []byte) (interface{}, error) {
	...
	return bson.Binary{Kind: byte(binKind), Data: binData}, nil
}
...
func jdecTimestamp(data []byte) (interface{}, error) {
	...
	return bson.MongoTimestamp(uint64(v.Func.T)<<32 | uint64(uint32(v.Func.I))), nil
}

func jencTimestamp(v interface{}) ([]byte, error) {
	ts := uint64(v.(bson.MongoTimestamp))
...
func jdecRegEx(data []byte) (interface{}, error) {
	...
	return bson.RegEx{v.Regex, v.Options}, nil
}
...
func jdecObjectId(data []byte) (interface{}, error) {
	...
	return bson.ObjectIdHex(v.Id), nil
}

func jencObjectId(v interface{}) ([]byte, error) {
	return fbytes(`{"$oid":"%s"}`, v.(bson.ObjectId).Hex()), nil
}
